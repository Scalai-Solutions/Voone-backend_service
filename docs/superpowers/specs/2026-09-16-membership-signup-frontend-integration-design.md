...
the _original_ `2a1e7eb`, not from the foundation commit — with the error/test foundation. It
...
in three commits: admin area, clinic dashboard, wallet template editor, a working QR _scanner_,
...
| points         | _none_                                        | `pointsBalance Int @default(0)`          |
| consent        | 4 columns                                     | _none_                                   |
...
- **`memberSince` and `privacyPolicyVersion` on Member are nullable.** Both are derived from
  the sign-up flow, and a member created outside it legitimately has neither; backfilling a
  consent version nobody was shown would manufacture false evidence.
- **`consentMarketing` and `consentSource` are required, with no default.** This is a change
  from the first draft, which defaulted both to accommodate existing rows — the pre-flight found
  `Clinic` and `Member` **empty**, so there is nothing to accommodate. A default would let an
  absent choice be read as a choice, and would quietly attribute a staff entry to the public
  form, which is exactly what these columns exist to prevent. `signUpMember` therefore takes
  the source as an explicit argument.

### 1.3 Migration SQL — the pre-flight came back clean

**Run before writing anything** (these were run; results below):
...
-- Rows that would fail the CHECK, i.e. how much a backfill owes.
...
ALTER TABLE "Member"
  ...
  CHECK ("phone" IS NULL OR "phone" ~ '^\+34[67][0-9]{8}
...
**Two assumptions here, flagged rather than buried.** Staff-entered members will _require_ a
...
Three hand-rolled CSS grids currently _look_ like QR codes and encode nothing —
...
2. **Merge `main` into `feat/membership-signup`**; resolve the three conflicts. Delete the old
   migration directory. A merge rather than a rebase: with twelve commits on the branch, a
   rebase replays the same three conflicts across several of them for no gain in a branch that
   merges as a unit. → `npm run lint && npm run typecheck`
...
Result: `Clinic`, `Member`, `ClinicTemplate` and `User` are **all empty** on Supabase; only the
three `TemplatePreset` rows exist. No duplicates, no non-conforming phones, nothing to backfill.

That removes the need for the defensive shape below, so the migration ships the simple one:
`slug` is added `NOT NULL` directly, and the CHECK is added **VALID** rather than `NOT VALID`.
If either assumption has since broken the migration fails loudly, which is the outcome to
want — silently grandfathering unnormalized numbers would make the unique index a lie.

<details><summary>The defensive version, kept for the day this runs against populated tables</summary>
