...
| `APPLE_WALLET_*`                      | Signing material: organization name, signer cert, signer key, key passphrase, WWDR cert. The pass type and team identifiers are read off the certificate, not set. The same certificate authenticates the APNs pushes that wake devices — there is no separate `.p8`. |
